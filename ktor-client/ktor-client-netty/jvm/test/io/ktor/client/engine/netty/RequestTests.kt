...
class RequestTests : TestWithKtor() {
    ...
    fun testReusingRequestBuilderOnMultipleClients() {
        ...
        val clientSuccess = HttpClient(Netty) {
            install(HttpTimeout) {
                requestTimeoutMillis = 1000
            }
        }
        val clientWithRequestTimeout = HttpClient(Netty) {
            install(HttpTimeout) {
                requestTimeoutMillis = 100
            }
        }
        val clientWithConnectTimeout = HttpClient(Netty) {
            install(HttpTimeout) {
    ...
    fun testRequestBodyPosting() {
        ...
        val response = HttpClient(Netty).use { client ->
            runBlocking {
