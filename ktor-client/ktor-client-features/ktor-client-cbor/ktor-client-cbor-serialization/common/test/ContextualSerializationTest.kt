...
class ContextualSerializationTest {
    ...
    fun testSerializationWithContext() {
        ...
        val contextual = KotlinxSerializer(Cbor { serializersModule = context })
        val simple = KotlinxSerializer()

        val data = UserData(1, "kotlin")

        val contextualBytes = contextual.writeContent(data)
        val simpleBytes = simple.writeContent(data)

        val makeInput = { ba: ByteArray ->
            buildPacket { ba.forEach { writeByte(it) } }
        }

        assertEquals(data, contextual.read(typeInfo<UserData>(), makeInput(contextualBytes)))
        assertEquals(data, simple.read(typeInfo<UserData>(), makeInput(simpleBytes)))
    }
}
