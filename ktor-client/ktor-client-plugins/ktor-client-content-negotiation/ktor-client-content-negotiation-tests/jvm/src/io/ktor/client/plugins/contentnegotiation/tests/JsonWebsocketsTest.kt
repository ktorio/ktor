...
abstract class JsonWebsocketsTest(private val converter: WebsocketContentConverter) {
    ...
    @Test
    open fun testJsonNullWithWebsocketsClient(): Unit = testServer {
    ...
    @Test
    open fun testJsonWithNullWebsocketsServer(): Unit = testServer {
        install(io.ktor.server.websocket.WebSockets) {
            contentConverter = this@JsonWebsocketsTest.converter
        }
        routing {
    ...
    @Test
    open fun testJsonWithWebsocketsClient(): Unit = testServer {
        install(io.ktor.server.websocket.WebSockets)
        routing {
            webSocket("/") {
                for (frame in incoming) {
    ...
    @Test
    open fun testJsonWithWebsocketsServer(): Unit = testServer {
        install(io.ktor.server.websocket.WebSockets) {
            contentConverter = this@JsonWebsocketsTest.converter
        }
        routing {
