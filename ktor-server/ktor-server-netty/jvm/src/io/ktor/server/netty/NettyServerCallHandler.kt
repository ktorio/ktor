...
internal class NettyServerCallHandler(
    ...
    private fun handleRequest(context: ChannelHandlerContext, call: PipelineCall) {
        ...
        currentJob = launch(callContext, start = CoroutineStart.UNDISPATCHED) {
            when {
                call is NettyHttp1ServerCall && !call.request.isValid() -> {
    ...
    private suspend fun respondError400BadRequest(call: NettyHttp1ServerCall) {
        logCause(call)
    ...
    private fun logCause(call: NettyHttp1ServerCall) {
        if (call.server.log.isTraceEnabled) {
            val cause = call.request.httpRequest.decoderResult()?.cause() ?: return
            call.server.log.trace("Failed to decode request", cause)
        }
    }
...
internal fun NettyHttp1ServerRequest.isValid(): Boolean {
