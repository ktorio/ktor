...
@Deprecated(message = "Renamed to NettyServerEngine", replaceWith = ReplaceWith("NettyServerEngine"))
public typealias NettyApplicationEngine = NettyServerEngine

/**
 * [ServerEngine] implementation for running in a standalone Netty
 */
public class NettyServerEngine(
    ...
    /**
     * Configuration for the [NettyServerEngine]
     */
    ...
    private val userContext = serverProvider().parentCoroutineContext +
        nettyDispatcher +
        NettyServerCallHandler.CallHandlerCoroutineName +
    ...
    init {
        ...
        pipeline.intercept(AFTER_CALL_PHASE) {
            (call as? NettyServerCall)?.finish()
        }
    }

    override fun start(wait: Boolean): NettyServerEngine {
