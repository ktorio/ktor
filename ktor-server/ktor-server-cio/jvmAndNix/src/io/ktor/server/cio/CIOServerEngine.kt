...
@Deprecated(message = "Renamed to CIOServerEngine", replaceWith = ReplaceWith("CIOServerEngine"))
public typealias CIOApplicationEngine = CIOServerEngine

/**
 * Engine that based on CIO backend
 */
public class CIOServerEngine(
    environment: ServerEnvironment,
    monitor: Events,
    developmentMode: Boolean,
    public val configuration: Configuration,
    private val serverProvider: () -> Server
) : BaseServerEngine(environment, monitor, developmentMode) {
    ...
    private suspend fun addHandlerForExpectedHeader(output: ByteWriteChannel, call: CIOServerCall) {
        ...
        call.request.pipeline.insertPhaseBefore(ServerReceivePipeline.Before, expectedHeaderPhase)
    ...
    private fun hasBody(request: CIOServerRequest): Boolean {
    ...
    private fun initServerJob(): Job {
        ...
        return CoroutineScope(
            serverProvider().parentCoroutineContext + engineDispatcher
