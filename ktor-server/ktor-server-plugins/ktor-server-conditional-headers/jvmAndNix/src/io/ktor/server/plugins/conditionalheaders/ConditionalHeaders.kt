...
public class ConditionalHeadersConfig {
    internal val versionProviders = mutableListOf<suspend (CallProperties, OutgoingContent) -> List<Version>>()
    ...
    /**
     * Registers a function that can fetch a version list for a given [CallProperties] and [OutgoingContent].
     *
     * @see [ConditionalHeaders]
     */
    public fun version(provider: suspend (CallProperties, OutgoingContent) -> List<Version>) {
        versionProviders.add(provider)
    }
}

internal val VersionProvidersKey: AttributeKey<List<suspend (CallProperties, OutgoingContent) -> List<Version>>> =
    AttributeKey("ConditionalHeadersKey")

/**
 * Retrieves versions such as [LastModifiedVersion] or [EntityTagVersion] for a given content.
 */
public suspend fun CallProperties.versionsFor(content: OutgoingContent): List<Version> {
