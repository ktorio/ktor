 ...
 */
public typealias CallIdProvider = (call: ServerCall) -> String?

/**
 * A function that verifies retrieved or generated call id. Should return `true` for a valid call id.
 * Also it could throw a [RejectedCallIdException] to reject an [ServerCall] otherwise an illegal call id
...
public class CallIdConfig {
    ...
    internal val responseInterceptors = ArrayList<(call: ServerCall, CallId: String) -> Unit>()
    ...
    /**
     * Allows you to retrieve a call ID from [ServerCall].
     ...
     * Rejects an [ServerCall] if [reject] is `true`; otherwise, an illegal call ID is ignored.
     *
     * Note that by default all retrieved/generated call IDs are verified using a default dictionary, which looks as follows:
     *
     * ```kotlin
    ...
    /**
     * Allows you to reply with a retrieved or generated call ID by modifying an [ServerCall].
     *
     * @see [replyToHeader]
     */
    public fun reply(block: (call: ServerCall, CallId: String) -> Unit) {
...
internal object CallIdSetup : Hook<suspend PipelineContext<Unit, PipelineCall>.(ServerCall) -> Unit> {
    override fun install(
        pipeline: ServerCallPipeline,
        handler: suspend PipelineContext<Unit, PipelineCall>.(ServerCall) -> Unit
    ) {
 ...
 */
public val ServerCall.callId: String? get() = attributes.getOrNull(CallIdKey)
