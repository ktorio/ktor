...
public class SamlAuthenticationProvider internal constructor(
    ...
    private val encryptionCredential: BasicX509Credential? = run {
        val encryptionCredential = spMetadata.encryptionCredential
        when {
            // Explicit encryption credential is set - validate it supports decryption
            encryptionCredential != null -> {
                val keyAlgorithm = encryptionCredential.privateKey?.algorithm ?: "unknown"
                require(encryptionCredential.supportsDecryption) {
                    "Encryption credential must use an RSA key for SAML assertion decryption. " +
                        "The configured encryption credential uses $keyAlgorithm algorithm. " +
                        "SAML encryption uses RSA-OAEP for key transport."
                }
                encryptionCredential
            }
            // Fall back to signing credential if it supports decryption
            signingCredential != null && signingCredential.supportsDecryption -> signingCredential
            // Signing credential is set but doesn't support decryption
            signingCredential != null -> {
                logger.warn(
                    "Signing credential uses ${signingCredential.privateKey?.algorithm ?: "unknown"} algorithm " +
                        "which cannot be used for SAML assertion decryption. " +
                        "If the IdP sends encrypted assertions, decryption will fail. " +
                        "Configure a separate encryptionCredential with an RSA key to support encrypted assertions."
                )
                null
            }
    ...
    private val replayCache by lazy {
        config.replayCache ?: InMemorySamlReplayCache()
    }
    ...
    private val responseProcessor: SamlResponseProcessor by lazy {
        SamlResponseProcessor(
            ...
            decryptionCredential = encryptionCredential,
