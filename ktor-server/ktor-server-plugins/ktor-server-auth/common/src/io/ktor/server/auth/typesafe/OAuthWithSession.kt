...
public class OAuthWithSessionConfig<S : Any, P : Any> @PublishedApi internal constructor() {
    ...
    internal var sessionConfigBlock: (SessionsConfig.(SessionAuthScheme<S, P>) -> Unit)? = null
     ...
     *
     * @param configure configures the [Sessions] plugin. Use the provided typed session scheme with helpers such as
     * `cookie(it)` or `header(it)` so OAuth callback storage and protected route authentication use the same session
     * provider name.
     */
    public fun session(configure: SessionsConfig.(SessionAuthScheme<S, P>) -> Unit) {
 ...
 * @property sessionScheme typed Session authentication scheme used by protected routes.
 */
@ExperimentalKtorApi
public class OAuthWithSessionScheme<S : Any, P : Any> @PublishedApi internal constructor(
    ...
    internal val oauthScheme: DefaultAuthScheme<
        ...
        >,
    public val sessionScheme: SessionAuthScheme<S, P>,
    internal val sessionConfig: SessionsConfig.(SessionAuthScheme<S, P>) -> Unit,
    ...
    public companion object {
        ...
        internal fun <S : Any, P : Any> from(
            ...
            val sessionConfig = requireNotNull(config.sessionConfigBlock) {
                "Session configuration is required. Call session { cookie(it) } inside oauthWithSession."
            }
            ...
            val sessionScheme = buildSessionAuthScheme(
 ...
 *     name = "oauth",
 *     principalFactory = { session -> loadUserByToken(session.accessToken) }
 * ) {
...
public fun <S : Any, P : Any> Route.oauthCallback(
    ...
    install(Sessions) {
        scheme.sessionConfig(this, scheme.sessionScheme)
    }
    ...
    route.get(path) callback@{
        try {
            ...
            call.sessions.set(scheme.sessionScheme, session)
