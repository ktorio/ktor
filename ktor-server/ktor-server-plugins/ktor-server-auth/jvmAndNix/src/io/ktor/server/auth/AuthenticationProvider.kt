 ...
 */
public typealias ServerCallPredicate = (ServerCall) -> Boolean

/**
 * An authentication function that accepts and verifies credentials and returns a principal when verification is successful.
 */
public typealias AuthenticationFunction<C> = suspend ServerCall.(credentials: C) -> Principal?
...
public abstract class AuthenticationProvider(config: Config) {
    ...
    /**
     * Authentication filters specifying if authentication is required for a particular [ServerCall].
     *
     * If there is no filters, authentication is required. If any filter returns `true`, authentication is not required.
     */
    public val skipWhen: List<ServerCallPredicate> = config.filterPredicates ?: emptyList()
    ...
    public open class Config protected constructor(public val name: String?) {
         ...
         * If there is no filters, authentication is required. If any filter returns `true`, authentication is not required.
         */
        internal var filterPredicates: MutableList<ServerCallPredicate>? = null
         ...
         */
        public fun skipWhen(predicate: (ServerCall) -> Boolean) {
