...
/**
 * Authentication plugin supports pluggable mechanisms for checking and challenging a client to provide credentials
 *
...
public class Authentication private constructor(config: Configuration) {
    ...
    private val logger = KtorSimpleLogger(Authentication::class.simpleName!!)
    ...
    /**
     * Installable plugin for [Authentication].
     */
    public companion object Plugin : ApplicationPlugin<Application, Configuration, Authentication> {
        /**
         * Authenticate phase in that authentication procedures are executed.
         * Please note that referring to the phase is only possible *after* plugin installation.
         */
...
public fun Route.authenticate(
    ...
    application.plugin(Authentication).interceptPipeline(authenticatedRoute, configurationNames, optional = optional)
...
/**
 * Installs [Authentication] plugin if not yet installed and invokes [block] on it's config.
