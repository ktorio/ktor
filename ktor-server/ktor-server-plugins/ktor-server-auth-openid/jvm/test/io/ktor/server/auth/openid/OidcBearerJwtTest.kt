...
class OidcBearerJwtTest {
    ...
    fun `principal serialization uses stable token serial names`() {
        val json = Json {
            serializersModule = OidcPrincipal.serializersModule
        }
        val serializer = PolymorphicSerializer(OidcPrincipal::class)

        val accessToken = json.encodeToString(serializer, OidcPrincipal.AccessToken("access-token"))
        val idToken = json.encodeToString(
            serializer,
            OidcPrincipal.IdToken(
                idToken = "id-token",
                userInfo = OidcPrincipal.UserInfo(subject = "user"),
            )
        )
        ...
        application {
            ...
            val google = oidc.provider(
                ...
                transformPrincipal = { p ->
                    val accessToken = p as? OidcPrincipal.AccessToken
