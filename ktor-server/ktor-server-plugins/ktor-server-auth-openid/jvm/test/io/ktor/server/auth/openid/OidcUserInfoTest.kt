...
class OidcUserInfoTest {
    ...
    fun `buildVerifiedPrincipal accepts signed userinfo and validates subject`() = testApplication {
        ...
        val userInfoBody = AtomicReference(keys.token(audience = "client-id", subject = "userinfo-user"))
    ...
    fun `buildVerifiedPrincipal rejects invalid signed userinfo`() = testApplication {
        ...
        val invalidCases = listOf(
            otherKeys.token(audience = "client-id", subject = "userinfo-user") to "signature",
            keys.token(audience = "client-id", subject = "userinfo-user", issuer = "https://issuer.example.net") to
                "issuer",
            keys.token(audience = "other-client", subject = "userinfo-user") to "audience",
            keys.token(audience = "client-id", subject = "other-user") to "UserInfo subject mismatch",
            keys.token(
                audience = "client-id",
                subject = "userinfo-user",
                algorithm = SignatureAlgorithm.RSA_SHA_384,
            ) to
                "allowed algorithms",
        )
    ...
    private fun ApplicationTestBuilder.installUserInfoVerifier(keys: OpenIdTestKeys) {
            ...
            routing {
                get("/verify") {
                    val result = runCatching {
                        oidcProvider.buildVerifiedPrincipal(
                            idToken = keys.token(audience = "client-id", subject = "userinfo-user"),
