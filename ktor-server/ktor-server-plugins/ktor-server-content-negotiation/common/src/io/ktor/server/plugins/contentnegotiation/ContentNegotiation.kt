 ...
 *
 * [Report a problem](https://ktor.io/feedback/?fqname=io.ktor.server.plugins.contentnegotiation.ContentTypeWithQuality)
 *
 * @param contentType
 * @param quality
 */
...
public val ContentNegotiation: RouteScopedPlugin<ContentNegotiationConfig> = createRouteScopedPlugin(
    ...
    application.attributes[DefaultContentTypesAttribute] = pluginConfig.registrations.map { it.contentType }
}

/**
 * Detects a suitable charset for an application call by using the `Accept` header or fallbacks to [defaultCharset].
 *
 * [Report a problem](https://ktor.io/feedback/?fqname=io.ktor.server.plugins.contentnegotiation.suitableCharset)
 */
