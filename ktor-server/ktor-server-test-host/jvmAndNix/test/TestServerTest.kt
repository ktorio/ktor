...
class TestServerTest {
    ...
    private val TestPlugin = createServerPlugin("test_plugin") {
        onCall { call ->
    ...
    private val testApplication = TestServer {
        install(TestPlugin)
        routing {
            get("a") {
                call.respond("OK")
    ...
    @Test
    fun testApplicationBlock() = testServer {
        application {
            install(TestPlugin)
            routing {
                get("a") {
    ...
    @Test
    fun testExternalServices() = testServer {
        externalServices {
    ...
    @Test
    fun testCanAccessClient() = testServer {
        class TestPluginConfig {
            lateinit var pluginClient: HttpClient
        }

        val TestPlugin = createServerPlugin("test", ::TestPluginConfig) {
            onCall {
    ...
    @Test
    fun testManualStart() = testServer {
    ...
    @Test
    fun testManualStartAndClientCallAfter() = testServer {
        externalServices {
            hosts("https://test.com") {
                routing {
                    get {
        ...
        routing {
            get {
                call.respond(client.get("https://test.com").bodyAsText())
            }
        }
    ...
    @Test
    fun testClientWithTimeout() = testServer {
    ...
    @Test
    fun testMultipleParallelRequests() = testServer {
        routing {
            get("/") {
                call.respondText("OK")
            }
    ...
    @Test
    fun testRequestRunningInParallel() = testServer {
        routing {
            post("/") {
    ...
    @Test
    fun testExceptionThrowsByDefault() = testServer {
        routing {
            get("/boom") {
    ...
    @Test
    fun testExceptionRespondsWith500IfFlagSet() = testServer {
        environment {
            config = MapServerConfig(CONFIG_KEY_THROW_ON_EXCEPTION to "false")
        }
        routing {
            get("/boom") {
                throw IllegalStateException("error")
    ...
    @Test
    fun testConnectors(): Unit = testServer {
        engine {
            connector {
                port = 8080
            }
    ...
    @Test
    fun testStartupJobsCompletion() = testServer {
        startApplication()
        withTimeoutOrNull(1000) {
            engine.server.coroutineContext.job.children.forEach { scope -> scope.join() }
    ...
    private fun testSocketTimeoutRead(timeout: Long, expectException: Boolean) = testServer {
        routing {
            get {
                call.respond(
                    HttpStatusCode.OK,
