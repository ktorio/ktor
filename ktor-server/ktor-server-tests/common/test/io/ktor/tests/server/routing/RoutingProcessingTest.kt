...
class RoutingProcessingTest {
    @Test
    fun testRoutingOnGETFooBar() = testSuspend {
        withTestApplication {
            application.routing {
                get("/foo/bar") {
    ...
    @Test
    fun testRoutingOnGETUserWithParameter() = testSuspend {
        withTestApplication {
            var username = ""
            application.routing {
                route("user") {
    ...
    @Test
    fun testRoutingOnGETUserWithSurroundedParameter() = testSuspend {
        withTestApplication {
            var username = ""
            application.routing {
                get("/user-{name}-get") {
    ...
    @Test
    fun testRoutingOnParamRoute() = testSuspend {
        withTestApplication {
    ...
    @Test
    fun testRoutingOnOptionalParamRoute() = testSuspend {
        withTestApplication {
            var selectedRoute = SelectedRoute.None
            application.routing {
                route("test") {
    ...
    @Test
    fun testRoutingOnRoutesWithSameQualityShouldBeBasedOnOrder() = testSuspend {
        // `accept {}` and `param {}` use quality = 1.0
        withTestApplication { // `accept {}` and `param {}` use quality = 1.0
            // `accept {}` and `param {}` use quality = 1.0
            var selectedRoute = SelectedRoute.None
            application.routing {
                route("paramFirst") {
    ...
    @Test
    fun testMostSpecificSelected() = testSuspend {
        withTestApplication {
            var path = ""
            application.routing {
                get("{path...}") {
                    path = call.parameters.getAll("path")?.joinToString<String>("/") ?: "/"
                }
                get("z/{path...}") {
                    path = "[Z] " + (call.parameters.getAll("path")?.joinToString<String>("/") ?: "/")
                }
            }
    ...
    @Test
    fun testRoutingOnGETUserUsernameWithInterceptors() = testSuspend {
        withTestApplication {
            ...
            on("handling GET /user/john") {
                ...
                assertTrue(
                    userNameGotWithinInterceptor,
                    "should have processed /user/username in context of interceptor"
                )
    ...
    @Test
    fun testRouteWithTypedBody(): Unit = testSuspend {
        withTestApplication {
            application.routing {
                post<String> { answer ->
    ...
    @Test
    fun testInterceptionOrderWhenOuterShouldBeAfter() = testSuspend {
        withTestApplication {
            var userIntercepted = false
            var wrappedWithInterceptor = false
            ...
            on("handling GET /user/john") {
                ...
                assertTrue(userIntercepted, "should have processed interceptor on /user node")
                assertFalse(
                    routingInterceptorWrapped,
                    "should have processed nested routing interceptor in a prior phase"
                )
    ...
    @Test
    fun testInterceptionOrderWhenOuterShouldBeBeforeBecauseOfPhase() = testSuspend {
        withTestApplication {
            var userIntercepted = false
            var wrappedWithInterceptor = false
            var rootIntercepted = false
            ...
            on("handling GET /user/john") {
                ...
                assertTrue(
                    routingInterceptorWrapped,
                    "should have processed nested routing interceptor in an after phase"
                )
                assertTrue(rootIntercepted, "should have processed root interceptor")
                assertEquals(userName, "john", "should have processed get handler on /user/username node")
            }
    ...
    @Test
    fun testInterceptionOrderWhenOuterShouldBeBeforeBecauseOfOrder() = testSuspend {
        withTestApplication {
            var userIntercepted = false
            var wrappedWithInterceptor = false
            var rootIntercepted = false
    ...
    @Test
    fun testInterceptReceivePipeline() = testSuspend {
        withTestApplication {
            var userIntercepted = false
            var wrappedWithInterceptor = false
            var rootIntercepted = false
    ...
    @Test
    fun testAcceptHeaderProcessing() = testSuspend {
        withTestApplication {
            application.routing {
                route("/") {
                    accept(ContentType.Text.Plain) {
                        handle {
                            call.respond<String>("OK")
                        }
                    }
            ...
            handleRequest(HttpMethod.Get, "/") {
                addHeader(HttpHeaders.Accept, "text/plain")
            }.let<TestApplicationCall, Unit> { call ->
                assertTrue { call.requestHandled }
                assertEquals<String?>("OK", call.response.content)
            }
            handleRequest(HttpMethod.Get, "/") {
                addHeader(HttpHeaders.Accept, "application/json")
            }.let<TestApplicationCall, Unit> { call ->
                assertTrue { call.requestHandled }
                assertEquals<String?>("{\"status\": \"OK\"}", call.response.content)
            }
            handleRequest(HttpMethod.Get, "/") {
            }.let<TestApplicationCall, Unit> { call ->
                assertTrue { call.requestHandled }
            ...
            handleRequest(HttpMethod.Get, "/") {
                ...
                assertEquals<HttpStatusCode?>(HttpStatusCode.BadRequest, call.response.status())
            }
            Unit
        }
    }
    ...
    @Test
    fun testTransparentSelectorWithHandler() = testSuspend {
        withTestApplication {
            application.routing {
                route("") {
                    transparent {
            ...
            handleRequest(HttpMethod.Get, "/").let<TestApplicationCall, Unit> { call ->
                assertTrue(call.requestHandled)
                assertEquals<String?>("OK", call.response.content)
            }
            Unit
    ...
    @Test
    fun testHostAndPortRoutingProcessing(): Unit = testSuspend {
        withTestApplication {
            application.routing {
                route("/") {
                    host("my-host", 8080) {
            ...
            handleRequest {
                ...
                assertEquals<String?>("host = my-host, port = 8080", call.response.content)
            }
            handleRequest {
                uri = "/1"
                method = HttpMethod.Get
                ...
                assertEquals<String?>(null, call.response.content)
            }
            handleRequest {
                uri = "/2"
                method = HttpMethod.Get
                ...
                assertEquals<String?>("host = my-host2, port = 9090", call.response.content)
            }
            handleRequest {
                uri = "/3"
                method = HttpMethod.Get
                ...
                assertEquals<String?>("host = my-host.net, port = 9091", call.response.content)
            }
            handleRequest {
                uri = "/3"
                method = HttpMethod.Get
                ...
                assertEquals<String?>("host = www.my-host.net, port = 9092", call.response.content)
            }
            handleRequest {
                uri = "/3"
                method = HttpMethod.Get
            ...
            handleRequest {
                ...
                assertEquals<String?>("host = my-host.net, port = 9093", call.response.content)
            }
            handleRequest {
                uri = "/4"
                method = HttpMethod.Get
                ...
                assertEquals<String?>("host = www.my-host.net, port = 9094", call.response.content)
            }
            Unit
        }
    }
    ...
    @Test
    fun testLocalPortRouteProcessing() = testSuspend {
        withTestApplication {
            application.routing {
                route("/") {
    ...
    @Test
    fun testRoutingWithTracing() = testSuspend {
        withTestApplication {
...
""",
    ...
    @Test
    fun testRouteWithParamaterPrefixAndSuffixHasMorePriority() = testSuspend {
        withTestApplication {
            application.routing {
                get("/foo:{baz}") {
