...
public value class SegmentedPath internal constructor(
    ...
    override fun iterator(): Iterator<String> = listIterator(0)

    override fun listIterator(): ListIterator<String> = listIterator(0)

    override fun listIterator(index: Int): ListIterator<String> {
        // We need a materialized list once a caller asks for a ListIterator (random access
        // backwards isn't possible from the raw string alone). This path is intentionally
        // kept for slow-path compatibility and is not hit by the fast-path resolver.
        return toMaterializedList().listIterator(index)
    }

    override fun subList(fromIndex: Int, toIndex: Int): List<String> {
        require(fromIndex in 0..toIndex) { "fromIndex=$fromIndex, toIndex=$toIndex" }
        return toMaterializedList().subList(fromIndex, toIndex)
    }

    private fun toMaterializedList(): List<String> {
