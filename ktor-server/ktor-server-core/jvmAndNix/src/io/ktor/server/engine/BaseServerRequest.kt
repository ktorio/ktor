...
@Deprecated(message = "Renamed to BaseServerRequest", replaceWith = ReplaceWith("BaseServerRequest"))
public typealias BaseApplicationRequest = BaseServerRequest

/**
 * Base class for implementing [PipelineRequest]
 */
public abstract class BaseServerRequest(final override val call: PipelineCall) : PipelineRequest {
    ...
    override val pipeline: ServerReceivePipeline = ServerReceivePipeline(
        call.server.developmentMode
    ).apply {
        resetFrom(call.server.receivePipeline)
    }
