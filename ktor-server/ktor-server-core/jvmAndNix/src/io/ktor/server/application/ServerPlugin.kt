...
@Deprecated(message = "Renamed to BaseServerPlugin", replaceWith = ReplaceWith("BaseServerPlugin"))
public typealias BaseApplicationPlugin<S, T, U> = BaseServerPlugin<S, T, U>
 ...
 * @param TPlugin is the instance type of the Plugin object
 */
public interface BaseServerPlugin<
    in TPipeline : Pipeline<*, PipelineCall>,
    out TConfiguration : Any,
    TPlugin : Any
    > : Plugin<TPipeline, TConfiguration, TPlugin>

@Deprecated(message = "Renamed to ServerPlugin", replaceWith = ReplaceWith("ServerPlugin"))
public typealias ApplicationPlugin<T> = ServerPlugin<T>

/**
 * Defines a [Plugin](https://ktor.io/docs/plugins.html) that is installed into Application.
 * @param TConfiguration is the configuration object type for this Plugin
 */
public interface ServerPlugin<out TConfiguration : Any> :
...
public fun <A : Pipeline<*, PipelineCall>, F : Any> A.plugin(plugin: Plugin<*, *, F>): F {
    ...
    } ?: throw MissingServerPluginException(plugin.key)
}
...
private fun <B : Any, F : Any> RoutingNode.installIntoRoute(
    ...
    if (server.pluginRegistry.getOrNull(plugin.key) != null) {
        throw DuplicatePluginException(
    ...
    val fakePipeline = when (this) {
        is RoutingRoot -> RoutingRoot(server)
...
@Deprecated(
    message = "Renamed to MissingServerPluginException",
    replaceWith = ReplaceWith("MissingServerPluginException")
)
public typealias MissingApplicationPluginException = MissingServerPluginException
...
public class MissingServerPluginException(
    public val key: AttributeKey<*>
) : IllegalStateException(), CopyableThrowable<MissingServerPluginException> {
    override val message: String get() = "Application plugin ${key.name} is not installed"

    override fun createCopy(): MissingServerPluginException = MissingServerPluginException(key).also {
