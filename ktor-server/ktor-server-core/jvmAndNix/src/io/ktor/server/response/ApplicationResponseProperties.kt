 ...
 */
public fun ServerResponse.header(name: String, value: String): Unit = headers.append(name, value)

/**
 * Appends a header with the specified [name] and [value] to a response.
 */
public fun ServerResponse.header(name: String, value: Int): Unit = headers.append(name, value.toString())

/**
 * Appends a header with the specified [name] and [value] to a response.
 */
public fun ServerResponse.header(name: String, value: Long): Unit = headers.append(name, value.toString())

/**
 * Appends the `E-Tag` header with the specified [value] to a response.
 */
public fun ServerResponse.etag(value: String): Unit = header(HttpHeaders.ETag, value)

/**
 * Appends the `Cache-Control` header with the specified [value] to a response.
 */
public fun ServerResponse.cacheControl(value: CacheControl): Unit =
 ...
 * Appends the `Content-Range` header with the specified [range] and [fullLength] to a response.
 */
public fun ServerResponse.contentRange(
    range: LongRange?,
    fullLength: Long? = null,
    unit: RangeUnits
) {
