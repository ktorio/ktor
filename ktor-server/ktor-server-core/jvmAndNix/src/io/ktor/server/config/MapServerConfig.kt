...
@Deprecated(message = "Renamed to MapServerConfig", replaceWith = ReplaceWith("MapServerConfig"))
public typealias MapApplicationConfig = MapServerConfig

/**
 * Mutable application config backed by a hash map
 */
public open class MapServerConfig : ServerConfig {
    /**
    ...
    override fun property(path: String): ServerConfigValue {
        return propertyOrNull(path) ?: throw ServerConfigurationException(
    ...
    override fun configList(path: String): List<ServerConfig> {
        ...
        val size = map[combine(key, "size")] ?: throw ServerConfigurationException("Property $key.size not found.")
        return (0 until size.toInt()).map {
            MapServerConfig(map, combine(key, it.toString()))
        }
    }
    ...
    override fun propertyOrNull(path: String): ServerConfigValue? {
        ...
        return if (!map.containsKey(key) && !map.containsKey(combine(key, "size"))) {
            ...
            MapServerConfigValue(map, key)
        }
    }

    override fun config(path: String): ServerConfig = MapServerConfig(map, combine(this.path, path))
     ...
     */
    protected class MapServerConfigValue(
        ...
        override fun getList(): List<String> {
            val size =
                map[combine(path, "size")] ?: throw ServerConfigurationException("Property $path.size not found.")
