 ...
 */
public fun ServerRequest.isMultipart(): Boolean = contentType().match(ContentType.MultiPart.Any)

/**
 * Gets a request's `User-Agent` header value.
 */
public fun ServerRequest.userAgent(): String? = header(HttpHeaders.UserAgent)

/**
 * Gets a request's `Cache-Control` header value.
 */
public fun ServerRequest.cacheControl(): String? = header(HttpHeaders.CacheControl)

/**
 * Gets a request's host value without a port.
 * @see [port]
 */
public fun ServerRequest.host(): String = origin.serverHost

/**
 * Gets a request's port extracted from the `Host` header value.
 * @see [host]
 */
public fun ServerRequest.port(): Int = origin.serverPort

/**
 * Gets ranges parsed from a request's `Range` header value.
 */
public fun ServerRequest.ranges(): RangesSpecifier? =
    header(HttpHeaders.Range)?.let { rangesSpec -> parseRangesSpecifier(rangesSpec) }

/**
 * Gets a request's URI, including a query string.
 */
public val ServerRequest.uri: String get() = origin.uri

/**
 * Gets a request HTTP method possibly overridden using the `X-Http-Method-Override` header.
 */
public val ServerRequest.httpMethod: HttpMethod get() = origin.method

/**
 * Gets a request's HTTP version.
 */
public val ServerRequest.httpVersion: String get() = origin.version
