...
class ServerPluginTest {
    ...
    fun `test empty plugin does not break pipeline`(): Unit = withTestApplication {
        val plugin = createServerPlugin("F", createConfiguration = {}) {
        }
    ...
    fun `test plugin with single interception`() {
        ...
        val plugin = createServerPlugin("F", createConfiguration = { Config() }) {
            onCall { call ->
                if (this@createServerPlugin.pluginConfig.enabled) {
    ...
    fun `test multiple installs changing config`() {
        val pluginF = createServerPlugin("F", { ConfigWithData() }) {
            onCall { call ->
    ...
    @Test
    fun `test routing scoped install can access route`() = testServer {
