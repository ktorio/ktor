...
public class StaticContentConfig<Resource : Any> internal constructor() {
    ...
    internal var modifier: suspend (Resource, Call) -> Unit = { _, _ -> }
     ...
     * For resources, [Resource] is a [URL] to a requested resource.
     */
    public fun modify(block: suspend (Resource, Call) -> Unit) {
        modifier = block
    }
 ...
 */
public fun Call.isStaticContent(): Boolean = parameters.contains(pathParameterName)
...
private suspend fun Call.respondStaticResource(
    index: String?,
    basePackage: String?,
    compressedTypes: List<CompressedFileType>?,
    contentType: (URL) -> ContentType,
    cacheControl: (URL) -> List<CacheControl>,
    modifier: suspend (URL, Call) -> Unit,
    exclude: (URL) -> Boolean,
    extensions: List<String>,
    defaultPath: String?
) {
