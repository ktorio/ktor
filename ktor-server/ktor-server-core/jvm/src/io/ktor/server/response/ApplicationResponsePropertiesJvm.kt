 ...
 */
public fun ServerResponse.header(name: String, date: Temporal): Unit =
    headers.append(name, date.toHttpDateString())

/**
 * Append response `Last-Modified` HTTP header value from [dateTime]
 */
public fun ServerResponse.lastModified(dateTime: ZonedDateTime): Unit = header(HttpHeaders.LastModified, dateTime)

/**
 * Append response `Expires` HTTP header [value]
 */
public fun ServerResponse.expires(value: LocalDateTime): Unit = header(HttpHeaders.Expires, value)
