...
public interface SchemaReflectionAdapter {
    public fun getName(type: KType): String? =
        ...
        else (type as? KClass<*>)?.qualifiedName
...
public class ReflectionJsonSchemaInference(
    ...
    private fun buildSchemaInternal(type: KType, visiting: MutableSet<KType>, includeAnnotations: List<Annotation> = emptyList()): JsonSchema {
        ...
        try {
            ...
            primitiveSchemaOrNull(kClass, includeAnnotations)?.let { base ->
            ...
            return jsonSchemaFromAnnotations(
                annotations = kClass.annotations,
                reflectSchema = ::schemaRefForClass,
                type = JsonType.OBJECT,
                title = adapter.getName(type),
    ...
    private fun primitiveSchemaOrNull(kClass: KClass<*>, annotations: List<Annotation>): JsonSchema? = when (kClass) {
        String::class, Char::class -> jsonSchemaFromAnnotations(annotations, ::schemaRefForClass, type = JsonType.STRING)
        ...
        // Java time
        java.time.Instant::class -> jsonSchemaFromAnnotations(annotations, ::schemaRefForClass, type = JsonType.STRING, format = "date-time")
        java.time.OffsetDateTime::class -> jsonSchemaFromAnnotations(annotations, ::schemaRefForClass, type = JsonType.STRING, format = "date-time")
        java.time.LocalDate::class -> jsonSchemaFromAnnotations(annotations, ::schemaRefForClass, type = JsonType.STRING, format = "date")
        java.time.LocalDateTime::class -> jsonSchemaFromAnnotations(annotations, ::schemaRefForClass, type = JsonType.STRING, format = "date-time")

        // Kotlinx datetime
        Instant::class -> jsonSchemaFromAnnotations(annotations, ::schemaRefForClass, type = JsonType.STRING, format = "date-time")
        LocalDate::class -> jsonSchemaFromAnnotations(annotations, ::schemaRefForClass, type = JsonType.STRING, format = "date")
        LocalDateTime::class -> jsonSchemaFromAnnotations(annotations, ::schemaRefForClass, type = JsonType.STRING, format = "date-time")
