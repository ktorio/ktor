...
import io.ktor.network.quic.errors.TransportError.*
...
internal object FrameReader {
    suspend inline fun readFrame(
        ...
        maxCIDLength: UInt8,
        onError: (QuicTransportError, FrameType) -> Unit,
    ) {
        val payload = packet.payload ?: error("Expected packet with payload") // programming error

        if (payload.isEmpty) {
            onError(PROTOCOL_VIOLATION, FrameType.PADDING)
            return
        }
        ...
        if (type == null) {
            onError(FRAME_ENCODING_ERROR, FrameType.PADDING)
            return
        }

        val err = when (type) {
            FrameType.PADDING -> processor.acceptPadding(packet)
            FrameType.PING -> processor.acceptPing(packet)

            FrameType.ACK -> readAndProcessACK(
                processor = processor,
                packet = packet,
                payload = payload,
            ...
            FrameType.ACK_ECN -> readAndProcessACK(
                processor = processor,
                packet = packet,
                payload = payload,
                ack_delay_exponent = transportParameters.ack_delay_exponent,
            ...
            FrameType.RESET_STREAM -> readAndProcessResetStream(processor, packet, payload)
            FrameType.STOP_SENDING -> readAndProcessStopSending(processor, packet, payload)
            FrameType.CRYPTO -> readAndProcessCrypto(processor, packet, payload)
            FrameType.NEW_TOKEN -> readAndProcessNewToken(processor, packet, payload)

            FrameType.STREAM,
            FrameType.STREAM_FIN,
            FrameType.STREAM_LEN,
            FrameType.STREAM_LEN_FIN,
            FrameType.STREAM_OFF,
            FrameType.STREAM_OFF_FIN,
            FrameType.STREAM_OFF_LEN,
            FrameType.STREAM_OFF_LEN_FIN,
            -> readAndProcessStream(processor, packet, payload, type)

            FrameType.MAX_DATA -> readAndProcessMaxData(processor, packet, payload)
            FrameType.MAX_STREAM_DATA -> readAndProcessMaxStreamData(processor, packet, payload)
            FrameType.MAX_STREAMS_BIDIRECTIONAL -> readAndProcessMaxStreamsBidirectional(processor, packet, payload)
            FrameType.MAX_STREAMS_UNIDIRECTIONAL -> readAndProcessMaxStreamUnidirectional(processor, packet, payload)
            FrameType.DATA_BLOCKED -> readAndProcessDataBlocked(processor, packet, payload)
            FrameType.STREAM_DATA_BLOCKED -> readAndProcessStreamDataBlocked(processor, packet, payload)
            FrameType.STREAMS_BLOCKED_BIDIRECTIONAL -> readAndProcessStreamsBlockedBidirectional(
                processor,
                packet,
                payload
            )

            FrameType.STREAMS_BLOCKED_UNIDIRECTIONAL -> readAndProcessStreamsBlockedUnidirectional(
                processor,
                packet,
                payload
            )

            FrameType.NEW_CONNECTION_ID -> readAndProcessNewConnectionId(processor, packet, maxCIDLength, payload)
            FrameType.RETIRE_CONNECTION_ID -> readAndProcessRetireConnectionId(processor, packet, payload)
            FrameType.PATH_CHALLENGE -> readAndProcessPathChallenge(processor, packet, payload)
            FrameType.PATH_RESPONSE -> readAndProcessPathResponse(processor, packet, payload)
            FrameType.CONNECTION_CLOSE_TRANSPORT_ERR -> readAndProcessConnectionCloseWithTransportError(
                processor,
                packet,
                payload
            )

            FrameType.CONNECTION_CLOSE_APP_ERR -> readAndProcessConnectionCloseWithAppError(
                processor,
                packet,
                payload
            )

            FrameType.HANDSHAKE_DONE -> processor.acceptHandshakeDone(packet)
        }
    ...
    private inline fun ByteReadPacket.readFrameType(): FrameType? {
        return FrameType.fromByte(readUInt8 { return null })
    }
}
